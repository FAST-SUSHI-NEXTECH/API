 ...
 * /user/upgrade:
 *   put:
 *     summary: Update a user permission to picker 
...
const putUserToPicker = async (req, res) => {
    try {
        const connection = await pool.getConnection();
        const { user_id } = req.body;

        // Use a parameterized query to prevent SQL injection
        const query = `UPDATE user SET permission = 2  WHERE id_user = ?`;
...
module.exports = { putUserToPicker };